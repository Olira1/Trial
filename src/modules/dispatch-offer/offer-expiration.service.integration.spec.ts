...
describe('OfferExpirationService (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    service = moduleRef.get(OfferExpirationService);
  });
  ...
  it('expires an overdue pending offer and atomically releases driver and request', async () => {
    const { driverId, requestId, offer } = await createPendingOffer();
    await makeOfferOverdue(offer.id);

    const expired = await service.expire(offer.id);

    expect(expired.state).toBe('expired');
    expect(expired.respondedAt).toBeInstanceOf(Date);
    ...
    expect(await countEventType(offer.id, 'dispatch_offer.expired.v1')).toBe(1);
  });

  it('returns the expired snapshot for duplicate expiration', async () => {
    const { offer } = await createPendingOffer();
    await makeOfferOverdue(offer.id);

    const first = await service.expire(offer.id);
    const second = await service.expire(offer.id);
  ...
  it('rejects a pending offer that is not yet overdue', async () => {
    const { offer } = await createPendingOffer();

    await expect(service.expire(offer.id)).rejects.toBeInstanceOf(
      ConflictException,
    );
  });
  ...
  it('rejects when the request is no longer offered', async () => {
    const { requestId, offer } = await createPendingOffer();
    await makeOfferOverdue(offer.id);
    await db
      .update(rideRequest)
  ...
  it('makes concurrent duplicate expirations idempotent', async () => {
    const { offer } = await createPendingOffer();
    await makeOfferOverdue(offer.id);

    const results = await Promise.all([
      service.expire(offer.id),
      service.expire(offer.id),
    ]);
